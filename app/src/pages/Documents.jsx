...
const Documents = ({ setCurrentRoute }) => {
    //TODO: *2: Este código abaixo temporario para testar dados locais
    ...
    // Funciona com usuario não logado, mas preso ao usuario 11:
    // const { data, error, isLoading } = useSWR(`http://localhost:3001/document?user_id=11&page=${page}&limit=${limit}`, fetcher, { refreshInterval: 5000 })

    // console.log("qual usuario??", user)
...
// TODO: tentando reparo erro de usuario não logado:
    // useEffect(() => {
    //     if (!data) {
    //     navigate('/login');
    //     }
...
// Cabeçaçho das colunas
    const columns = [
        ...
        { headerName: 'Ações', key: null, id: false, action: (params) => {//não precisa passar key, pois o próprio componente ListViewer já faz isso(??)
            return <>
    ...
    return <>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} md={10}>
                        ...
                        { !error && data !== undefined ? <ListViewer {...props} /> : error ?  "Um erro ocorreu" : "Nenhum dado encontrado"  }
                        </Stack>
                    </Grid>
                    <Grid item xs={0} md={1}></Grid>
                </Grid>
