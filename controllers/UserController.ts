...
class UserController{
    ...
    async register(username: string, name: string, email: string, password: string){
        ...
        // Enviar e-mail de confirmação. json é o context com as variáveis que serão usadas no template (name, token e url)
        //tem que estar alinhado com os campos de util\Email.ts:
        ...
        this.mail.sendEmail(email, 'Confirmação de e-mail', 'token-email', {
            name,
            token,
            url: process.env.MAIL_CONFIRMATION_URL
        })
        return true;
    }
